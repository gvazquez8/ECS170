...
model.load_state_dict(torch.load("latest_model.pth", map_location='cpu'))
...
for frame_idx in range(1, num_frames + 1):
    ...
    if done:
        ...
        s = str(frame_idx) + "," + str(episode_reward) + "\n"
        rewards_to_write.append(s)
        episode_reward = 0
    ...
    if len(replay_buffer) > replay_initial:
        ...
        if frame_idx % 10 == 0 and len(losses) >= 10:
            mean_loss = np.mean(losses[-10:], 0)[1]
            s = str(frame_idx) + "," + str(mean_loss) + "\n"
            losses_to_write.append(s)
    ...
    if frame_idx % 50000 == 0:
        ...
        with open("rewards.csv", "a") as f:
            f.writelines(rewards_to_write)
            rewards_to_write = []
        with open("losses.csv", "a") as f:
            f.writelines(losses_to_write)
            losses_to_write = []